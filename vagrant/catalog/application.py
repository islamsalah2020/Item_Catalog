...
# Show All Catergories and latest items
@app.route('/')
@app.route('/catalog')
def showCategories():
    # Get all categories
    ...
    return render_template('categories.html', categories=session.query(
        Category).all(), categoryItems=session.query(CategoryItem).all())
